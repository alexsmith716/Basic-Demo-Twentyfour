...
const About = () => {
	...
	useEffect(() => {
			...
			if (toggleCustomerState) {
				console.log('>>>>>>>>>>>>>>>>>>>>>>>> About > useEffect() > componentDidUpdate > toggleCustomerState: ', toggleCustomerState);
			}

			//	componentWillUnmount
			return () => {
				//	some effects might require cleanup
				console.log('>>>>>>>>>>>>>>>>>>>>>>>> About > useEffect() > componentWillUnmount > cleanup phase');
			};
		},
		[toggleCustomerState]
	);
	...
	return (
		...
		<>
			...
			<div className="container">
				...
				<h2 className="color-salmon font-tester-font2 mb-3">Demo Component State By Toggling Our Customers!</h2>

				<div className={`${toggleCustomerState ? 'mb-4' : 'mb-5'}`}>
					<Button type="button" className={`btn-${toggleCustomerState ? 'success' : 'warning'}`} onClick={() => setToggleCustomerState(!toggleCustomerState)}>
						{toggleCustomerState ? 'They\'re All Here!' : 'Now they\'re Gone! But just make sure they always keep coming back!'}
					</Button>
				</div>
